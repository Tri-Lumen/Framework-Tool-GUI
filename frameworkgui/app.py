...
def windows_cpu_brand():
    """The CPU's marketing name from the registry, or '' anywhere else.

    Windows has no /proc/cpuinfo, and %PROCESSOR_IDENTIFIER% is a CPUID
    dump — "AMD64 Family 25 Model 116 Stepping 1, AuthenticAMD" — not a
    name. ProcessorNameString is the string the vendor actually ships
    ("AMD Ryzen 7 7840U w/ Radeon 780M Graphics"), and it is readable
    without elevation.
    """
    try:
...
def detect_cpu():
    ...
    Two names come back: `label`, the full string, which the CPU limits
    pane shows because that pane is about this exact chip; and `short`, the
    trimmed one for the Overview heading, where the full string wrapped the
    sub-line onto three lines on a real machine.
    """
    cpuinfo = ""
    try:
    ...
    brand = windows_cpu_brand() if IS_WINDOWS else ""
    label = power.cpu_label(cpuinfo, ident, brand)
    return {
        "vendor": power.detect_vendor(cpuinfo, ident, platform.machine()),
        "label": label,
        "short": power.short_cpu_label(label),
    }
...
class App(QMainWindow):
    ...
    def _build_fallback_strip(self, parent):
        ...
        row.setContentsMargins(12, 0, 12, 0)
        # Worded for the reason: "cannot composite" is true on a Windows 10
        # box but unhelpful on a Windows 11 one where the fix is a Settings
        # toggle the user owns.
        self.fallback_label = label(
            backdrop.unavailable_message(self.compositing_reason), "caption",
            strip)
        row.addWidget(self.fallback_label)
        row.addStretch(1)
    ...
    def _apply_appearance(self):
        ...
        # Both attributes, together: the translucent one gives the window an
        # alpha channel, and without WA_NoSystemBackground Qt still fills
        # the client area with the palette's window brush first, which is
        # opaque and sits between the backdrop and everything above it.
        if self.testAttribute(Qt.WA_TranslucentBackground) != acrylic:
            self.setAttribute(Qt.WA_TranslucentBackground, acrylic)
            self.setAttribute(Qt.WA_NoSystemBackground, acrylic)
    ...
    def _apply_backdrop(self):
        """Ask Windows for the backdrop the current appearance implies.

        Kept apart from `_apply_appearance` because it has to run again at
        moments that have nothing to do with the appearance changing:
        toggling WA_TranslucentBackground makes Qt destroy and recreate the
        native window, and the recreated window is a fresh HWND carrying
        none of the DWM attributes. Same on show. Applying it once from
        `__init__`, as this used to, set the attribute on a handle the
        window no longer had by the time it appeared.

        A refusal is taken at face value: if DWM will not give us the
        backdrop, the app stops claiming it has one rather than showing a
        translucent surface over nothing.
        """
        if not IS_WINDOWS or not self.isVisible():
            # Before the first show there is no window to attribute, and
            # asking for winId() here would create the native one early —
            # which is half of how the attributes went missing.
            return
        wanted = self.appearance == theme.ACRYLIC and self.compositing
        if backdrop.apply_windows_backdrop(int(self.winId()), wanted):
            return
        if not wanted:
            return
        self.compositing = False
        self.compositing_reason = backdrop.OLD_WINDOWS
        self.appearance = theme.OPAQUE
        self.fallback_label.setText(
            backdrop.unavailable_message(self.compositing_reason))
        self.fallback_strip.setVisible(True)
        self._apply_appearance()

    def showEvent(self, event):
        super().showEvent(event)
        # After the window is on screen, and after any native-window
        # recreation Qt did on the way there. Deferred by one event-loop
        # turn because that recreation can happen during the show itself.
    ...
    def _device_subline(self):
        """The one line under the device name — trimmed to be readable.

        Every field here arrives longer than it is useful: the CPU comes
        back as a CPUID dump on Windows, and an EC version carries the
        commit hashes of three firmwares, its build timestamp and the
        hostname of the machine that built it. Untrimmed, on a real Laptop
        13, this wrapped to three lines and said almost nothing. The full
        strings are still one hover away — see `_device_detail`.
        """
        parts = []
        board = self.caps.get("model", "")
        if board and board != "Unknown":
            parts.append(board)
        if self.cpu.get("short") or self.cpu.get("label"):
            parts.append(self.cpu.get("short") or self.cpu["label"])
        for key, name in (("ec", "EC"), ("bios", "BIOS")):
            if self.firmware.get(key):
                parts.append("{} {}".format(
    ...
    def _device_detail(self):
        ...
        rows = [("Board", self.caps.get("model", "")),
                ("CPU", self.cpu.get("label", "")),
                ("EC", self.firmware.get("ec", "")),
                ("BIOS", self.firmware.get("bios", "")),
    ...
    def _page_ports(self, box, parent):
        self._heading(
            ...
            "--pdports-chromebook on an EC without it. Both describe the "
            "mainboard port, not the card in front of it: USB-C and USB-A "
            "cards are passive, so only --dp-hdmi-info and "
            "--audio-card-info ever name a module.")
        panel = widgets.Panel(parent)
    ...
    # except for Detail, widened from 100 because the honest wordings for a
    # port that reported a voltage and no current do not fit in 100px, and
    # a clipped reading is worse than a wider column.
    PORT_COLUMNS = (40, 0, 110, 90, 150)
    ...
    def _render_ports(self):
        ...
        for index, port in enumerate(ports):
            ...
            self.port_rows.addWidget(self._port_row((
                ...
                ("{:.1f} W".format(watts) if watts else "—", "cellmono"),
                (self._port_detail(port), "caption"),
            )))

    @staticmethod
    def _port_detail(port):
        """What a port is carrying, in as many words as the EC gave us.

        Four different facts, and the difference between them matters: a
        port with nothing in it, a port whose contract is fully reported, a
        port reporting a voltage but no current (which is what the charging
        port on a real Laptop 13 does), and a port that says only that
        something is attached. Collapsing the last two into "no PD
        contract", as this used to, is how a machine that was charging
        showed four idle bays.
        """
        if not port_attached(port):
            return "nothing attached"
        volts, ma = port.get("volts"), port.get("ma")
        if volts and ma:
            return "{:.1f} V {:.1f} A".format(volts, ma / 1000.0)
        if volts:
            return "{:.1f} V · no current".format(volts)
        charging = port.get("charging")
        if charging:
            return "attached · {} charging".format(charging.lower())
        return "attached · no figures"

    @staticmethod
    def _bay_power(port):
        """The same reading as `_port_detail`, at bay-row width."""
        watts = port_watts(port)
        if watts:
            return "{:.1f} W".format(watts)
        if port.get("volts"):
            return "{:.1f} V".format(port["volts"])
        if port.get("charging"):
            return "{} charging".format(port["charging"].lower())
        return "power not reported"
    ...
    def _fill_bays(self):
        ...
        for index, (icon, name, detail) in enumerate(self.module_rows):
            ...
            attached = port_attached(port)
            if attached and role == "sink":
        ...
        if source and hasattr(self, "bay_source"):
            # Named for what it is: the port's own state. Neither command
            # can see the expansion card in front of the port, so a bay with
            # a USB-C card and nothing plugged into it reads as idle — which
            # looked like a failure to detect the card until it said so.
    ...
    PASSIVE_CARDS_NOTE = ("USB-C and USB-A cards are passive passthroughs — "
                          "nothing reports them, so a bay reads idle until "
    ...
    def _fill_cards(self):
        ...
        And the cards that are *not* identifiable are worth a sentence of
        their own. A USB-C card is a passive passthrough: there is no
        device on it to enumerate, so the only thing either port command
        can see is the mainboard port behind it. Four populated bays
        reading "nothing attached" looks like broken detection until the
        pane says why.
        """
        ...
        self.cards_line.setText(
            ("Also fitted: {} · the CLI cannot say which bay. {}".format(
                " · ".join(cards), self.PASSIVE_CARDS_NOTE) if cards
             else "No DP/HDMI or Audio card reported. {}".format(
                 self.PASSIVE_CARDS_NOTE)))
        self.cards_line.setWordWrap(True)
    ...
    def tool_input_power(self):
        ...
        if rc2 == 0:
            for p in parse_ports(pdout):
                if not (port_attached(p) and p["role"].lower() == "sink"):
                    continue
                if port_watts(p):
                    self._append(
                    ...
                    # Attached and drawing, but the EC did not fill in the
                    # numbers. Saying nothing here read as "no adapter".
                    self._append(
                        "Port {}: sink, but the EC reported no contract "
                        "figures\n".format(p["port"]))
            ...
            if port_is_live(p):
                ...
                self._append("{}: {:6s} {} {:.1f} W "
                             "({:.1f} V / {} mA)\n".format(
                                 ...
                                 p["volts"], p["ma"]))
            elif port_attached(p):
                # Something is on this port; the EC just did not say how
                # much it is carrying. Reporting that as "nothing
                # negotiated" is what made a charging port read as idle.
                self._append("{}: {:6s} attached, {}\n".format(
                    title, p["role"], self._bay_power(p)))
            else:
                self._append("{}: {:6s} nothing attached\n".format(
                    title, p["role"]))
